...
class QFunction(chainer.Chain):
    def __init__(self, obs_size, n_actions, n_hidden_channles=2):
        super(QFunction, self).__init__()
        with self.init_scope():
            self.l1 = L.Linear(obs_size, n_hidden_channles)
            self.l2 = L.Linear(n_hidden_channles, n_hidden_channles)
            self.l3 = L.Linear(n_hidden_channles, n_actions)
