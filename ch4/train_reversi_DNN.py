...
class Board():
    ...
    def board_reset(self):
        self.board = np.zeros((SIZE, SIZE), dtype=np.float32) # 全ての石をクリア。ボードは２次元配列(i,j)で定義する
        ...
        for di, dj, in DIR:
            ...
            while 0 <= i < SIZE and 0 <= j < SIZE:  # (i,j)座標が盤面内に収まっている間繰り返す
                ...
                if 0 <= i < SIZE and 0 <= j < SIZE and boardcopy[i,j] == opp: # 盤面に収まっており、かつ相手の石だったら
                    ...
                    break
                elif boardcopy[i,j] == self.turn and flag == True:  # 自分と同じ色の石が来れば挟んでいるのでリバース処理を確定
    ...
    # ゲーム終了チェック
    def end_check(self):
        if np.count_nonzero(self.board) == SIZE * SIZE or self.pss == 2:    # ボードに全て石が埋まるか、双方がパスしたら
            self.game_end = True
...
def main():
    ...
    gamma = 0.99
    # ε-greedy法
    explorer = chainerrl.explorers.LinearDecayEpsilonGreedy(start_epsilon=1.0, end_epsilon=0.1, decay_steps=50000, random_action_func=board.random_action)
    ...
    agent_black = chainerrl.agents.DQN(q_func, optimizer, replay_buffer_b, gamma, explorer, replay_start_size=1000, minibatch_size=128, update_interval=1, target_update_interval=1000)
    agent_white = chainerrl.agents.DQN(q_func, optimizer, replay_buffer_b, gamma, explorer, replay_start_size=1000, minibatch_size=128, update_interval=1, target_update_interval=1000)
